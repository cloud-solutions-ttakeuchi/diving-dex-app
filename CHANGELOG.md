...
- **Cross-Linking Proposals (App)**:
    - **Discovery Report**: 生物詳細ページから「発見報告」ボタンを追加。目撃したスポットを検索して、出現頻度（レアリティ）とともに紐付けを提案可能に。
    - **Add Species**: スポット詳細ページから「生物を追加」ボタンを追加。その場所で確認された生物を検索して、出現頻度とともに紐付けを提案可能に。
    - **Pending Visibility**: 自分が提案した未承認の紐付けデータも詳細画面のリストに「提案中」ラベル付きで表示されるよう対応。

### Improved
- **Wikipedia Image Integration (App)**: 
    - Wikipedia画像検索において、パス解決の不備を修正し、画像プレビューおよびメタデータの取得を安定化。
- **UI/UX (App)**:
    - 各詳細画面のリスト表示に「提案中」バッジを追加。
    - 空の状態（データなし）の際、ユーザーに投稿を促すガイドUIを追加。
- **Development Policy**: 
    - プロジェクト専用の開発ルール `.agent/rules.md` を導入し、AIアシスタントの行動指針を明確化。

### Fixed
- **Type Safety (App)**: 複数コレクションを跨ぐデータ結合時の TypeScript 型エラーを解消。
