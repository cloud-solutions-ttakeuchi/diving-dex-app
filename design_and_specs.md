...
## 1. 概要
**Dive-Dex App**（通称：ダイビングログアプリ）は、ダイバーが自身のダイビング活動を記録し、発見した生物をコレクションとして管理できる「ログブック × 生物図鑑」を融合したWebアプリケーションです。
...
- **テーマカラー**: 海をイメージした **Ocean Blue** (`bg-ocean`, `text-deepBlue`) を基調とし、清潔感と没入感を演出。
- **レスポンシブ**: モバイル利用（現地でのログ付け）を想定したレスポンシブデザイン。
- **インタラクション**: マイクロアニメーション（Hover時の拡大、Fade-in等）によるリッチな操作感。
...
| `VERTEX_AI_DATA_STORE_ID` | Vertex AI Search データストア ID | 高度な検索に使用するデータソースの ID を指定します。 |
