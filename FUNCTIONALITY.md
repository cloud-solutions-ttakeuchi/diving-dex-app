...
- **相互リンク提案 (Discovery & Link)**:
  - **発見報告 (Creature Detail)**: その生物を目撃したスポットを検索し、出現頻度を指定して報告可能。
  - **生物追加 (Spot Detail)**: そのスポットで確認された生物を検索し、出現頻度を指定して報告可能。
  - **ステータス管理**: 一般ユーザーの投稿は `pending`（提案中）として保存され、管理者の承認後に正式に紐付けられる。
