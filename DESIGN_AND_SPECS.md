...
- **オフライン・ファースト設計 (App)**:
  - **Network Status Indicator**: ネットワーク未接続時、画面上部に「オフラインモード」バッジをフローティング表示。
  - **Optimistic Saving**: オフライン時の保存操作に対し、「端末に保存完了」という明確なフィードバックを返し、ユーザーの不安を解消。
  - **Data Resilience**: ネットワーク復帰時の自動同期により、保存ボタンを押し直す必要のないシームレスな体験。
